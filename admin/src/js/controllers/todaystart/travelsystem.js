app.controller('TravelSystemCtrl', ['$scope', "$http", "$timeout",
    ...
    function($scope, $http, $timeout, $stateParams, $state, $localStorage, $window) {
        ...
        var getDataList = function() {
            ...
            var q = $http({
                method: "GET",
                url: _Api + "/admin/travelsystem/list",
                params: params
            });
        ...
        //     });
        // }
        $scope.ckTravelSystemDelete = function(id) {
            var user_id = id;
            if (!!user_id) {
                layer.confirm("确定要删除当前记录吗？", {
                    ...
                    $http({
                        method: "POST",
                        url: _Api + "/admin/travelsystem/delete",
                        data: {
                            id: user_id
                        }
