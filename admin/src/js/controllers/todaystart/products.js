app.controller('ProductsCtrl', ['$scope', "$http", "$state", "$localStorage", function($scope, $http, $state, $localStorage) {
    var page_size = 10;
    $scope.keyword = "";
    ...
    $scope.ckDelete = function(id) {
        if (!!id) {
            layer.confirm("确定要删除当前的产品吗？", {
                btn: ["确定", "取消"]
            }, function() {
                $http({
                    method: "POST",
                    url: _Api + "/admin/product/delete",
                    data: {
                        id: id
                    }
