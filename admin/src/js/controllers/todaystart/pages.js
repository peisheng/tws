app.controller('PagesCtrl', ['$scope', "$http", "$state", "$localStorage", function($scope, $http, $state, $localStorage) {
    var page_size = 10;
    $scope.keyword = "";
    ...
    $scope.ckDelete = function(id) {
        if (!!id) {
            layer.confirm("确定要删除当前的页面吗？", {
                btn: ["确定", "取消"]
            }, function() {
                $http({
                    method: "POST",
                    url: _Api + "/admin/page/delete",
                    data: {
                        id: id
                    }
